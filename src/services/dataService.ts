...
/** Make a string safe to use as a Firestore document id (no slashes etc). */
...
export const addStudent = (data: Omit<Student, 'id' | 'createdAt'>) =>
...
const studentImportKey = (student: {
  fullName: string;
  parentPhone?: string;
  studentClass?: string;
}) =>
  `${cleanText(student.fullName).toLowerCase()}__${cleanText(
    student.parentPhone || student.studentClass
  ).toLowerCase()}`;
...
export const importStudents = async (
  ...
  existingStudents.forEach((student) => {
    ...
    if (student.fullName && !existingByKey.has(key)) {
  ...
  rows.forEach((row, index) => {
    const payload: Omit<Student, 'id' | 'createdAt'> = {
      ...
      studentClass: cleanText(row.studentClass),
      parentName: cleanText(row.parentName),
      parentPhone: cleanText(row.parentPhone),
      parentEmail: cleanText(row.parentEmail),
    ...
    if (seenInFile.has(key)) {
      result.skipped += 1;
      result.errors.push(
        `Dòng ${index + 2}: trùng học sinh trong file (${payload.fullName})`
      );
      return;
    }
    seenInFile.add(key);
    ...
    existingByKey.set(key, { id: studentRef.id, ...payload });
    result.created += 1;
    operations.push({
  ...
  // Mỗi lô tối đa 240 học sinh. Nếu có classId thì mỗi học sinh dùng
  // hai thao tác ghi: một document học sinh và một document enrollment.
  for (const chunk of chunkArray(operations, 240)) {
    const batch = writeBatch(db);

    chunk.forEach((operation) => {
      if (operation.create && operation.studentRef) {
        batch.set(operation.studentRef, {
          ...operation.payload,
          createdAt: serverTimestamp(),
        });
      }
      ...
      if (classId) {
        batch.set(doc(db, 'enrollments', `${classId}__${operation.studentId}`), {
...
export const getClassRoster = async (classId: string): Promise<Student[]> => {
  ...
  const studentIds = enr.docs.map((d) => d.data().studentId as string);
...
export const enrollStudent = (studentId: string, classId: string) =>
  setDoc(doc(db, 'enrollments', `${classId}__${studentId}`), {
    studentId,
    classId,
    createdAt: serverTimestamp(),
  });
...
/**
 * Tự chuyển điểm cũ trong collection /scores sang sổ điểm mới.
 * Chỉ chạy khi sổ điểm mới chưa có cột nào, nên không tạo dữ liệu trùng.
 */
...
// ===================================================================
//  PARENT REPORT (public read by studentId)
// ===================================================================
export const getStudentReport = async (
  ...
  for (const cid of classIds) {
    ...
    // Nếu đã có sổ điểm mới thì ưu tiên dữ liệu mới để tránh hiện trùng điểm cũ đã migrate.
